...
class UserProfile {
				...
				callback: function(r) {
					if(r.message.leave_allocation){
						...
						<table class="table table-bordered small" style="margin: 0px 0px 20px 0px;">
							<thead style="background-color: rgba(102, 255, 102, 0.8);">
								<tr>
	...
	employee_asset_dashboard() {
		let $asset_dashboard = this.wrapper.find('.asset-dashboard');
		if (this.user_id) {
			frappe.call({
				method: "frappe.desk.page.user_profile.user_profile.get_asset_details",
				async: false,
				args: {
					user: this.user_id,
				},
					...
					if (r.message.length != 0){
						let html = $(__(`
							<table class="table table-bordered small" style="margin: 0px 0px 20px 0px;">
								<thead style="background-color: rgba(102, 255, 102, 0.8);">
									<tr>
										<th style="width: 20%" class="text-left">${__('Asset Code')}</th>
										<th style="width: 20%" class="text-left">${__('Asset Name')}</th>
										<th style="width: 20%" class="text-left">${__('Issued Date')}</th>
										<th style="width: 20%" class="text-left">${__('Amount')}</th>
									</tr>
								</thead>
								<tbody style="background-color: rgb(245, 242, 242);">
								`));
								for(const asset of r.message) {
									html.append($(__(`
										<tr>
											<td class="text-left">${asset.name}</td>
											<td class="text-left">${asset.asset_name}</td>
											<td class="text-left">${frappe.datetime.str_to_user(asset.purchase_date)}</td>
											<td class="text-left">${format_currency(asset.gross_purchase_amount)}</td>
										</tr>
									`)));
								}
							...
							`)));
						$asset_dashboard.append(html);
					} 
					else {
						let html = $(__(`<p style="margin-top: 10px; color:red;"> No asset issued. </p>`));
						$asset_dashboard.append(html);
					}
				}
			});
			...
			frappe.call({
				...
				callback: function(r) {
					if(r.message.length > 0){
						let html = $(__(`
						<table class="table table-bordered small" style="margin: 0px 0px 20px 0px;">
							<thead style="background-color: rgba(217, 255, 102, 0.8);">
								<tr>
